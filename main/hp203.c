...
static esp_err_t hp203_read_reg(hp203_handle_t sensor, uint8_t reg, uint8_t *data)
{
    ...
    ret = i2c_master_start(cmd);
    assert(ESP_OK == ret);
    ret = i2c_master_write_byte(cmd, (sens->dev_addr << 1) | I2C_MASTER_READ, true);
    assert(ESP_OK == ret);
    ret = i2c_master_read(cmd, data, 1, I2C_MASTER_LAST_NACK);
    assert(ESP_OK == ret);
    ...
    // ret = i2c_master_write_read_device(sens->bus, sens->dev_addr, &command, 1, data, 1, HP203_I2C_TIME_OUT / portTICK_PERIOD_MS);
...
static esp_err_t hp203_write_reg(hp203_handle_t sensor, uint8_t reg, uint8_t data)
{
    ...
    ret = i2c_master_write_byte(cmd, (sens->dev_addr << 1) | I2C_MASTER_WRITE, true);
    assert(ESP_OK == ret);
    ret = i2c_master_write_byte(cmd, HP203_CMD_WRITE_REG | reg, true);
    assert(ESP_OK == ret);
    ...
    // ret = i2c_master_write_to_device(sens->bus, sens->dev_addr, write_data, 2, HP203_I2C_TIME_OUT / portTICK_PERIOD_MS);
...
esp_err_t hp203_reset(hp203_handle_t sensor)
{
    ...
    // ret = i2c_master_write_to_device(sens->bus, sens->dev_addr, &command, 1, HP203_I2C_TIME_OUT / portTICK_PERIOD_MS);
...
esp_err_t hp203_read_pressure(hp203_handle_t sensor, float *pressure)
{
    ...
    ret = i2c_master_write_byte(cmd, (sens->dev_addr << 1) | I2C_MASTER_READ, true);
    assert(ESP_OK == ret);
    ret = i2c_master_read(cmd, data, 3, I2C_MASTER_LAST_NACK);
    assert(ESP_OK == ret);
    ret = i2c_master_stop(cmd);
    assert(ESP_OK == ret);
    ret = i2c_master_cmd_begin(sens->bus, cmd, 1000 / portTICK_PERIOD_MS);
...
esp_err_t hp203_read_pt(hp203_handle_t sensor, float *pressure, float *temperature)
{
    ...
    ret = i2c_master_write_byte(cmd, (sens->dev_addr << 1) | I2C_MASTER_READ, true);
    assert(ESP_OK == ret);
    ret = i2c_master_read(cmd, data, 6, I2C_MASTER_LAST_NACK);
    assert(ESP_OK == ret);
    ret = i2c_master_stop(cmd);
    assert(ESP_OK == ret);
    ret = i2c_master_cmd_begin(sens->bus, cmd, 1000 / portTICK_PERIOD_MS);
...
esp_err_t hp203_int_enable(hp203_handle_t sensor, uint8_t int_en)
{
    ...
    ret = hp203_write_reg(sens, HP203_REG_INT_EN, int_en);
